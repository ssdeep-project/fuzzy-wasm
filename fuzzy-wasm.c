...
struct fuzzy_state the_fuzzy_state;

struct blocksize_str_entry
{
  int len;
  const char *str;
};

struct blocksize_str_entry blocksize_str_entries[NUM_BLOCKHASHES] =
{
  {1, "3"},
  {1, "6"},
  {2, "12"},
...
void fuzzy_new(void)
{
  struct fuzzy_state *self = &the_fuzzy_state;
  self->bhstart = 0;
  self->bhend = 1;
...
int fuzzy_set_total_input_length(uint32_t len_lo, uint32_t len_hi)
{
  uint_least64_t total_fixed_length = ((uint_least64_t)len_hi << 32) | len_lo;
  struct fuzzy_state *state = &the_fuzzy_state;
  unsigned int bi = 0;
		 ...
		 unsigned int flags)
{
  ...
  i = blocksize_str_entries[bi].len;
  memcpy(result, blocksize_str_entries[bi].str, i);
  result[i] = ':';
  remain -= i + 1;
  result += i + 1;
...
//
static uint32_t csubstr_hashes[SPAMSUM_LENGTH - (ROLLING_WINDOW - 1)];
static struct roll_state csubstr_roll;
static int has_common_substring(const char *s1, size_t s1len, const char *s2, size_t s2len)
{
  ...
  memset(csubstr_hashes, 0, sizeof(csubstr_hashes));

  // first compute the windowed rolling hash at each offset in
  // the first string
  roll_init (&csubstr_roll);

  for (i = 0; i < ROLLING_WINDOW - 1; i++)
    roll_hash(&csubstr_roll, (unsigned char)s1[i]);
  for (i = ROLLING_WINDOW - 1; i < s1len; i++)
  {
    roll_hash(&csubstr_roll, (unsigned char)s1[i]);
    csubstr_hashes[i - (ROLLING_WINDOW - 1)] = roll_sum(&csubstr_roll);
  }
  ...
  // rolling hash and compare it to all of the rolling csubstr_hashes
  ...
  for (j = 0; j < ROLLING_WINDOW - 1; j++)
    roll_hash(&csubstr_roll, (unsigned char)s2[j]);
  for (j = 0; j < s2len - (ROLLING_WINDOW - 1); j++)
  {
    roll_hash(&csubstr_roll, (unsigned char)s2[j + (ROLLING_WINDOW - 1)]);
    uint32_t h = roll_sum(&csubstr_roll);
    for (i = 0; i < s1len; i++)
    {
      // confirm the match after checking potential match
      if (csubstr_hashes[i] == h && !memcmp(s1 + i, s2 + j, ROLLING_WINDOW))
	  return 1;
    }
  }
...
static unsigned long long parray[CHAR_MAX - CHAR_MIN + 1];
...
static int edist_t[2][EDIT_DISTN_MAXLEN+1];
...
static int edit_distn(const char *s1, size_t s1len, const char *s2, size_t s2len) {
  ...
  for (i2 = 0; i2 <= s2len; i2++)
    edist_t[0][i2] = i2 * EDIT_DISTN_REMOVE_COST;
...
static char s1b1[SPAMSUM_LENGTH], s1b2[SPAMSUM_LENGTH];
static char s2b1[SPAMSUM_LENGTH], s2b2[SPAMSUM_LENGTH];
static char *s1p, *s2p, *tmp;
...
static int read_blocksize(const char *str, size_t len, unsigned long *block_size)
{
  unsigned long bs = 0;
  unsigned long n;
  size_t i;
...
int fuzzy_compare(const char *str1, const char *str2)
{
  ...
  if (read_blocksize(str1, (size_t)(s1p - str1), &block_size1) != 1 ||
      read_blocksize(str2, (size_t)(s2p - str2), &block_size2) != 1) {
    return -1;
  }
...
// WebAssembly handles memory by multiple of 64k but fuzzy-wasm's memory
// requirement is much low. Since we don't waste memory, we will use
// some of unused areas for small buffer intended for digests.
#define TMP_BUFFER_SIZE 256
static char temp_buffer[FUZZY_MAX_RESULT * TMP_BUFFER_SIZE];
char* fuzzy_wasm_get_temp_buffer(void)
{
	return temp_buffer;
}
uint32_t fuzzy_wasm_get_temp_buffer_slot_size(void)
{
	return FUZZY_MAX_RESULT;
}
uint32_t fuzzy_wasm_get_temp_buffer_count(void)
{
